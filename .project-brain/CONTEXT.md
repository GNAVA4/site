...
## Stack
Python + Django 6.0.2, SQLite (dev), Pillow, django-cleanup, Bootstrap 5 (CDN; тонкая тема —
Фаза 3). Конфиг и email — через os.environ (без django-environ).
...
- Поиск — на стороне Python (store/search.py), НЕ в SQL: SQLite не умеет регистр кириллицы;
  rapidfuzz-Левенштейн ловит опечатки. На проде заменить на pg_trgm/SearchVector [user, ADR 003]
...
- settings.TELEGRAM_MANAGER больше НЕ используется кодом (наследие, удалить)
- Дефолтный SECRET_KEY — только dev; прод требует длинный DJANGO_SECRET_KEY
